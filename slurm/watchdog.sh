...
RUNS_NOW=$(ls $SCR/star/runs 2>/dev/null | wc -l)
LAST=$(cat $SCR/star/.watchdog_last 2>/dev/null || echo 0)
...
if [ "$RUNS_NOW" -ne "$LAST" ]; then
  ...
  bash $SCR/star/finalize.sh
  echo "$RUNS_NOW" > $SCR/star/.watchdog_last
else
  echo "nothing new since last publish"
fi

# Re-arm unless someone asked it to stop.
if [ ! -f $SCR/star/.watchdog_stop ]; then
  sbatch --begin=now+3hours $SCR/star/watchdog.sh
